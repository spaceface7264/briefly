...
- ✅ `FUNCTIONS_INVOKE_SECRET` set on the production Cloudflare Worker (`rainbow`) 2026-06-03, so prod org creation sends the org-admin invite email. The `notify-org-invite` function + `createOrg` invite flow are deployed and Resend delivery is verified end to end.
- ✅ `NEXT_PUBLIC_APP_URL` already wired in `wrangler.jsonc` (production = `https://app.briefly.dk`), so invite signup links use the real domain. No separate action was needed (already configured for Stripe onboarding).
